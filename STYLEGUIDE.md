...
- **Install hint (below sign-in):** `.auth-install-note` is a collapsed `<details>` rendered like a quiet footer hint, not a boxed accordion. Summary `.auth-install-note__summary` is centered, `textSm`, weight 500, secondary text, with a leading `smartphone` icon (`.auth-install-note__icon`). Expanded `.auth-install-note__platforms` shows left-aligned iPhone and Android instructions in one inset panel matching `.targets-level-row` (`colorBackgroundInset`, `colorDividerStrong`, radius md), with a horizontal divider between platforms. Each `.auth-install-note__platform` includes `.auth-install-note__platform-title` (`textSm`, weight 600, primary) and `.auth-install-note__platform-steps` (numbered `textSm`, primary). iOS share and Android kebab controls use `.auth-install-note__inline-icon` — a hairline-outlined inline icon treatment. No JS — `<details>`/`<summary>` provides the expand/collapse behavior.
